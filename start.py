...
def plot_web_graph():
    ...
    res = es.search(index="wikipedia", size=VectorBuild.get_MAX_ITEMCOUNT(), body={"query": {"match_all": {}}},
                    filter_path=['hits.hits._source.title', 'hits.hits._source.refer'])

    for cluster in res['hits']['hits']:
        print("\t" + cluster['_source']['refer'].replace("b'/wiki/", "").replace("'", "").replace("_", " ") + " → " +
              cluster['_source']['title'])
...
def start_kmeans():
    ...
    print(
        """
        Visualize clusters?[y/n]
        """)
    selected_task = input()
    VectorBuild.init(l, selected_task)
